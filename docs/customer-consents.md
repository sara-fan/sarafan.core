...
1. Apply migrations `20260908181115_0_0_7_CustomerConsents`, `20260908221440_0_0_9_ConsentListPaginationIndexes`, and `20260910221600_0_0_9_Login` through the established migration process. The first creates the versioned consent schema, drops the obsolete `customer_consents` table and rows without importing them, and creates `customer_consent_withdrawal_requests` directly in its final three-column form. The second changes only indexes for bounded queue and audit reads. The login migration introduces combined-authentication receipts, numeric customer state, and session token versioning; it deliberately does not rewrite or normalize existing phone values. No rights-case or rights-audit schema exists.
...
`legal_documents` preserves source bytes and canonical presentation. `consent_events` contains versioned decisions. `consent_associations` records the verified customer session that first observed a browser receipt without turning the browser decision into customer consent. `consent_replay_tombstones` prevents replay after evidence disposal. `consent_onboarding` contains a keyed phone digest and a short-lived, single-use authentication receipt binding the resolved flow, target account when applicable, exact documents, content hashes, decisions, and idempotency keys. Legal documents and legal-document audit events remain indefinitely.
