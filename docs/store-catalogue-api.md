...
|---|---|
| GET `/api/v1/stores?sort=recommended` | All Active and Priority stores |
...
Creation initializes the entire aggregate with equal `createdAt` and `updatedAt`; subsequent writes advance timestamp/version. Public catalogue queries omit Active and Priority rows missing a logo before the featured limit; staff can still inspect and repair such rows.
