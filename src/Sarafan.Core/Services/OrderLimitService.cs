...
{
    private BigInteger Numerator => new BigInteger(Eur.OfficialRate * 1000000m) * Usd.Nominal * 100000;
    ...
    internal OrderLimitCheckDto ToDto() => new(1000, Currency.Eur, true, Usd.SourceEffectiveDate, MaximumTotalUsd);
}
...
public sealed class OrderLimitService(AppDbContext database, TimeProvider timeProvider, ILogger<OrderLimitService> logger)
{
    ...
    internal static OrderLimitCheckDto ToDto(OrderLimitRatePair? pair)
        => pair?.ToDto() ?? new(1000, Currency.Eur, false, null, null);
