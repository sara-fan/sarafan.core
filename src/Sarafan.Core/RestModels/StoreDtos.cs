...
public sealed record StaffStoreDto(int Id, string Name, string Description, string OfficialUrl,
    StoreStatus Status, bool ShowOnHome, int DisplayOrder, DateTimeOffset CreatedAt, DateTimeOffset UpdatedAt,
...
public sealed record StoreLimitsDto(int NameMaxLength, int DescriptionMaxLength, int DescriptionRecommendedLength,
    ...
    int LogoMaxDimension, int LogoMaxPixels, int LogoMaxFrames, int LogoMaxAnimationPixels, int LogoMaxMetadataBytes);
