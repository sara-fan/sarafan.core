...
public sealed class Order
{
    ...
    internal void UpdatePricing(DateTimeOffset now, bool confirmed)
    {
        if (Status != OrderStatus.UnderReview) throw new InvalidOperationException("Only a quote under review can change.");
        var timestamp = NormalizeToPostgresTimestamp(now);
        UpdatedAt = timestamp > UpdatedAt ? timestamp : UpdatedAt.AddMicroseconds(1);
