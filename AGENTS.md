...
- Store writes use bounded multipart form data, explicit Version for update/delete, and one SaveChanges transaction for fields and logo. Hidden drafts may lack a logo; Active and Priority stores may not. Retain the old logo when no upload is supplied. Catalogue reads are independent of customer consent, FX and IANA availability; URL validation on writes uses the IANA catalogue. See `docs/store-catalogue-api.md` for the client contract.
- Initialize the complete store aggregate, including its optional logo, with equal CreatedAt/UpdatedAt and one initial Version. Reserve mutation methods for later changes. Public reads exclude missing logos before ordering/featured limiting, while staff reads retain repair access.
