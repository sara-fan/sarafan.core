...
- Store Ops exposes `officialUrlRules` using the shared URL-rules DTO and current IANA catalogue. Normalize store writes with the order address validator: assume HTTPS when omitted, preserve explicit HTTP(S), require a DNS hostname with an IANA TLD, and reject credentials, IPs, control characters and backslashes. Return `invalid_store_url` against `officialUrl`. Missing IANA data blocks Ops and writes with `tld_catalog_unavailable`; catalogue/detail/logo reads stay independent of it. Existing stored addresses remain readable for repair.

- FR-PUB-006–014 and Core #40 govern the store catalogue foundation. Store status is numeric Hidden (0, default), Active (1, catalogue) or Priority (2, catalogue and homepage). Ops owns Russian status labels and maxPriorityStores=6. DisplayOrder is nonnegative and globally unique, including Hidden stores. Keep logos in the separate one-to-one `store_logos` table with cascade deletion, and keep catalogue stores independent of order product snapshots.
- Mutate stores through their model methods with a server `TimeProvider` snapshot; field, status, selection, order and logo changes advance the opaque `Version` concurrency token and UTC microsecond `UpdatedAt`. Load the existing logo before replacement, retain the tracked dependent, and keep logo bytes out of list projections.
- Core #41 exposes anonymous `/api/v1/stores` reads and separate `/api/v1/backoffice/stores` management. All staff may view; only Administrator creates/deletes; Administrator and Shift manager edit, including publication and ordering. Enforce the distinct ViewStores/CreateStore/EditStore/DeleteStore actions at HTTP and service boundaries.
- Return Active and Priority stores publicly; featured reads select Priority only. Serialize all catalogue mutations with the shared transaction advisory lock before checking unique ordering and the six-Priority limit; updates exclude their own record. Return placement conflicts as structured displayOrder/status field errors. Recommended/featured order is DisplayOrder then Id; alphabetical sorts use case-insensitive ru-RU comparison in Core after projection, with ascending Id ties. Revalidate public responses and check current store visibility before evaluating conditional logo requests; never cache a hidden logo as publicly available.
- Store writes use bounded multipart form data, explicit Version for update/delete, and one SaveChanges transaction for fields and the required image. Creation requires an image for every status; update may omit the upload only to retain an existing image. Keep legacy image-less records staff-readable for repair, reject saving them without an image, and exclude them from public responses. Catalogue reads are independent of customer consent, FX and IANA availability; URL validation on writes uses the IANA catalogue. See `docs/store-catalogue-api.md` for the client contract.
...
- Per-service currencies and price methods are configurable: every service permits RUB/USD and all five methods. Percent always uses the USD merchandise base and produces a USD charge; Stepped boundaries use USD, while their charge currency remains configurable. Other Demo currency choices are configuration guidance, never service-specific validation restrictions. Only Product retains a dedicated input contract. DomesticDelivery (300) and CustomsPayments (800) use ordinary catalogue tariffs and numeric `manualAmounts` keys.
- Catalogue Ops owns currency symbols (`₽`, `$`) alongside RUB/USD values, names and aliases. Pricing clients display those symbols for monetary amounts and currency choices rather than deriving them from route aliases; unrelated EUR metadata stays unchanged.
...
- Fresh pricing schema seeds exactly one immutable Product (`service=0`) catalogue row: ID 1, Manual, USD, no amount or availability bounds. Its UTC timestamps and UUID version are generated when the schema is applied. Neither API nor database may create another Product row, modify/reclassify the seed, replace it, or delete it; all staff may read it. Other service definitions retain Administrator-only audited CRUD.
- Pricing reconciliation is pinned to `sara-fan/sarafan@981eb0fce77d92fde28e3270cc761e69c5925c48`, PRICE, as amended by the owner's clarifications, with manual-rate setup (FR-PRICE-042) deferred. Percent minimum/maximum values cap the resulting USD charge, not the merchandise price. Fixed contains an amount; Manual and Auto contain a currency but no catalogue amount. Auto values cannot be overridden by staff. Stepped (400) selects one flat charge using total USD merchandise: first `From` is null, following `From` equals preceding `By` and is exclusive, each `By` is inclusive, and final `By` is null. Preserve Percent=0, Fixed=100, Manual=200, Auto=300. Amounts use two decimal places and percentages four.
- Price components retain original currencies and distinguish calculated (including zero), not calculated and not applicable. Round each calculated component to two decimal places using half-up before summing. Confirmed calculations retain immutable component, parameter and official USD/RUB snapshots for their validity period and afterward. `ServiceKind.IsIncludedInTotal()` excludes DomesticDelivery and CustomsPayments from forecast/confirmed totals and includes other services; expose this read-only flag as `includedInTotal` in catalogue Ops, never as a stored/editable tariff field. Missing excluded amounts do not block confirmation. Catalogue mutations are Administrator-only; editable quote-specific inputs allow Administrator and Shift manager.
- Treat nullable `AvailableFrom` and `AvailableBy` as inclusive Moscow calendar dates; null starts/ends are unbounded. Same-service periods never overlap, including shared endpoints; retain both friendly application validation and the PostgreSQL GiST exclusion constraint. Serialize writes with the catalogue advisory transaction lock and require the opaque UUID version for update/delete.
...
- Manage back-office access control through `BackofficeAuthorization` in `src/Sarafan.Core/Authentication`. Keep the fixed role catalogue and deny-by-default action-to-role matrix there; unknown roles and actions must never grant access. HTTP endpoints use its registered `BackofficePolicies`, service guards use `RequireAllowed`, and Ops capabilities use `IsAllowed`. Add new permissions centrally rather than introducing feature-local role checks or authorization helpers.
...
- [Product review amendment v1.22](https://github.com/sara-fan/sarafan.spec/blob/main/spec/Product%20review%20amendment.md), Core #34, governs order products and staff corrections. Keep one authoritative current product on the order: store, name, USD unit price, quantity, color, size and comment. Do not add parallel submitted, recognized or override columns, fallback projections or precedence rules. Store image, dimensions, characteristics and commercial applied rate as current product metadata.
- Record accepted creation values and subsequent parser or staff results as append-only product audit events. Creation records the normalized product used for idempotency; parser events have no actor; staff events contain the actor and complete before/after products. A staff correction updates the current order product, advances `UpdatedAt`, records rate references and appends its audit event in one transaction.
- New products and every staff correction require name (500), positive USD price decimal(10,2), quantity 1–4, optional store/color/size (200 each) and comment (2000). Trim outer whitespace without changing internal text/case. Resolve equivalent original-payload idempotency from the creation audit before new-field/rate/quantity limits; retain current-value fallback for legacy rows without a creation audit and never compare later staff corrections with the replay request.
- Enforce the inclusive 900 EUR merchandise-only limit (1000 EUR × constant reserve coefficient 0.9) using the latest common official CBR USD/RUB and EUR/RUB date no later than Moscow today. Respect nominals and compare integer price cents/rate millionths with BigInteger cross-products, never rounded EUR. Missing pairs block new submissions/corrections but not reads or equivalent replays. Store each validation pair only on its product audit event; do not duplicate creation or correction rate references on `Order`. Keep the distinct commercial `AppliedExchangeRate` metadata separate.
- Staff product details use public orderNumber and current nullable buyer-profile fields, with no general customer API. EditOrderProduct is deny-by-default, permits the four established roles only in UnderReview, and may correct the current product without altering identity, URL, profile or status. UpdatedAt and Status are concurrency tokens; preserve timestamp microseconds in DTOs. Conflict responses require reload, never automatic overwrite.
- Both order Ops publish identical productLimits/valueLimit contracts; staff does not depend on customer Ops. Public preview remains stateless manual_review with nullable product prefill for future recognition. New clients and Core must ship together because new submissions require `product`. Keep product/profile/request data redacted in boundary logs.
