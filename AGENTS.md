...
- [Product review amendment v1.22](https://github.com/sara-fan/sarafan.spec/blob/main/spec/Product%20review%20amendment.md), Core #34, governs submitted products and staff corrections. Keep the recognized product and store, dimensions, characteristics and commercial applied rate server-owned. Store immutable submitted name/USD unit price/color/size separately from original URL/quantity/comment and from nullable complete staff overrides, including the editable store. Effective projections, staff search and sorting use overrides → submission/recognized store → available legacy snapshot; never infer color/size from arbitrary characteristics.
...
- Staff order details expose storeName/imageUrl/dimensions/characteristics as read-only recognition fields. Limit-check DTOs publish exceededMessage from the same coefficient-derived source as RFC 9457 errors; clients must not duplicate the reserve coefficient or message formula.
