...
- Return only Active stores publicly, with featured selection filtered before the six-item limit. Recommended/featured order is DisplayOrder then Id; alphabetical sorts use case-insensitive ru-RU comparison in Core after projection, with ascending Id ties. Revalidate public responses and check current store visibility before evaluating conditional logo requests; never cache a hidden logo as publicly available.
- Store writes use bounded multipart form data, explicit Version for update/delete, and one SaveChanges transaction for fields and logo. Hidden drafts may lack a logo; Active stores may not. Retain the old logo when no upload is supplied. Catalogue validation is independent of customer consent, FX and IANA availability. See `docs/store-catalogue-api.md` for the client contract.
