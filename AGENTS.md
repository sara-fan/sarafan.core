...
- Before enabling real orders or a real payment-system integration, replace and disable the predictable phone-suffix demo verification mechanism. A build/runtime environment named Production does not satisfy this requirement. While real operations are disabled, reject order routes before model binding or input validation with `resource_not_found`, and retain service-level guards for non-HTTP callers. Keep this release gate tracked in the [MVP delivery issue](https://github.com/sara-fan/sarafan.spec/issues/26).
...
- Orders store non-null UTC `CreatedAt` and `UpdatedAt` values initialized from the injected `TimeProvider`. `CreatedAt` is immutable after insertion; every domain mutation advances `UpdatedAt` without moving it backwards.
- Keep the staff order list read-only and protected by `ManualQuotes` plus the real-operations release gate. Validate its bounded page, one-sort, search, exact/group status and inclusive Moscow creation-date filters with `invalid_order_list_filter`; keep filtering, count, deterministic ordering with internal-ID tie-breakers, compact projection, `Skip` and `Take` in `IQueryable`. Never expose internal order/customer IDs or log searches, URLs, public order numbers, or filter contents.
