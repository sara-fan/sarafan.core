...
- Per-service currencies and price methods are configurable: every service permits RUB/USD and all five methods. Demo choices (such as RUB Percent commission) are configuration guidance, never service-specific validation restrictions. Tariff-driven calculations use the configured charge currency/method; retain separate product and domestic-delivery input contracts.
...
- Pricing reconciliation is pinned to `sara-fan/sarafan@981eb0fce77d92fde28e3270cc761e69c5925c48`, PRICE, as amended by the owner's open-start/inclusive-upper clarification, with manual-rate setup (FR-PRICE-042) deferred. Percent minimum/maximum values cap the resulting charge in its RUB/USD currency, not the merchandise price; the current Demo commission configuration uses RUB. Fixed contains an amount; Manual and Auto contain a currency but no catalogue amount. Auto values cannot be overridden by staff. Stepped (400) selects one flat charge using total merchandise in its independent interval currency: first `From` is null, following `From` equals preceding `By` and is exclusive, each `By` is inclusive, and final `By` is null. Preserve Percent=0, Fixed=100, Manual=200, Auto=300. Amounts use two decimal places and percentages four.
- Price components retain original currencies and distinguish calculated (including zero), not calculated and not applicable. Round each calculated component to two decimal places using half-up before summing. Confirmed calculations retain immutable component, parameter and official USD/RUB snapshots for their validity period and afterward. Domestic delivery and customs remain separate from forecast/confirmed totals. Catalogue mutations are Administrator-only; editable quote-specific inputs allow Administrator and Shift manager.
