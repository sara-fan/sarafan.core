...
- Authentication consent payloads belong to code requests. After successful code verification and before either login or receipt completion, reject `TermsAccepted = true`, a supplied `TermsDocumentId`, or a non-null `PersonalDataConsent` with `400 invalid_auth_request`; verification must never replace or supplement receipt-bound confirmations. Invalid codes retain `401 invalid_code` precedence.
