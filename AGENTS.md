...
- Core owns the exact and upper-level Russian names, route aliases, and terminal-state marker exposed by anonymous `GET /api/v1/orders/ops`. Clients consume this catalogue for grouping and presentation instead of maintaining their own mappings, and order statuses do not use a lookup table.
